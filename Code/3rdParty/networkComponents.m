...
isDiscovered = false(N,1);
% Empty members cell

% check every node
s       = sum(A,2);
sIdx    = find(s);
members = cell(1,numel(sIdx));
for n=1:numel(sIdx)
   
    if ~isDiscovered(sIdx(n))
        % started a new group so add it to members
        members{end+1} = sIdx(n);
        % account for discovering n
        isDiscovered(sIdx(n)) = true;
        % set the ptr to 1
        ptr = 1;
        while (ptr <= length(members{end}))
            ...
            newNbrs = nbrs(~isDiscovered(nbrs));
            % we can now mark them as discovered
            isDiscovered(newNbrs) = true;
...
nComponents = sum(cellfun(@numel,members)~=0);
sizes       = zeros(1,nComponents);
for n=1:nComponents
