...
rouseParams.numSimulations  = 3000; % number of simulations in each round
rouseParams.dimension       = 3;
rouseParams.numBeads        = 32;
rouseParams.b               = 3;
rouseParams.diffusionConst  = 1;
rouseParams.connectedBeads  = [1 16]; % the beads connected other than the trivial connections. given by bead pairs
rouseParams.encounterDist   = rouseParams.b/7;
...
for cIdx = 1:size(rouseParams.connectedBeads,1)
    rouseParams.springConst(rouseParams.connectedBeads(cIdx,1), rouseParams.connectedBeads(cIdx,2))=rouseParams.springConst(rouseParams.connectedBeads(cIdx,1), rouseParams.connectedBeads(cIdx,2))*5;
    rouseParams.springConst(rouseParams.connectedBeads(cIdx,2), rouseParams.connectedBeads(cIdx,1))=rouseParams.springConst(rouseParams.connectedBeads(cIdx,2), rouseParams.connectedBeads(cIdx,1))*5;
end

d                           = sqrt(2*rouseParams.diffusionConst*rouseParams.dt);
numSteps                    = (0.09)*(rouseParams.b^2)/(6*(d^2)*sin(pi/(2*rouseParams.numBeads))^2); % n times the number of steps until relaxation of the chain 
...
rouseParams.recipeFileName  = 'simpleRouseDebugRecipe';
