...
rouseParams.dt              = 0.3*(rouseParams.b^2)/(12*rouseParams.diffusionConst);
rouseParams.noiseSTD        = 0;%sqrt(2*rouseParams.diffusionConst*rouseParams.dt);
...
numSteps                    = (1)*(rouseParams.b^2)/(6*(d^2)*sin(pi/(2*rouseParams.numBeads))^2); % n times the number of steps until relaxation of the chain 
rouseParams.numSteps        = round(numSteps);
rouseParams.stiffConnectors = []; % fixed bead indices pairs for the number of loops
rouseParams.connectedBeads  = [1 64]; % the beads connected other than the trivial connections. given by bead pairs
...
rouseParams.plot            = false;  % TODO: should call the plotter  
