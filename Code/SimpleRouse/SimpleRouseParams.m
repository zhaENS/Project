...
rouseParams.encounterDist   = rouseParams.b/3;
...
numSteps                    = (0.1)*(rouseParams.b^2)/(6*(d^2)*sin(pi/(2*rouseParams.numBeads))^2); % n times the number of steps until relaxation of the chain 
rouseParams.numSteps        = round(numSteps);
rouseParams.stiffConnectors = []; % fixed bead indices pairs for the number of loops
rouseParams.connectedBeads  = [16 32]; % the beads connected other than the trivial connections. given by bead pairs
rouseParams.affineBeadsNum  = []; % a fixed pair indices of affine beads
rouseParams.kOff            = 0.1;% the detachment rate of affine beads
rouseParams.recipeFolder    = fullfile(pwd,'Code','SimpleRouse','Recipes');
rouseParams.recipeFileName  = 'simpleRouseDebugRecipe';%'simpleRouseconditionalEncounter3BeadsIn108BeadChainDynamicLoop';
