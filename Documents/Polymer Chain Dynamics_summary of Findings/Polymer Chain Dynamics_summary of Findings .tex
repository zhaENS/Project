...
These peaks represent a frequent encounter with a distal part of the chain. The list of bead numbers and the frequent encounter they have with the distal chain parts (represented by bead numbers) is summarized in Table \ref{table_peaksOfTheEncounterData}
% old table (One Sided)
\begin{table}[H]\label{table_peaksOfTheEncounterData}
\begin{tabular}{l l l}
...
% the figure below should be replaced
\begin{figure}[H]
...
The fitted exponent ($B$) and bias ($A$) values obtained by minimizing the error in the model \ref{equation_fitModel} are shown in Figure \ref{figure_fittedExpAndBias}
\begin{figure}[H]
...
\label{figure_fittedExpVsFittedBias}
\end{figure}
...
From Figure \ref{figure_fittedExpVsFittedBias} we see that the bias values tend to increase positively with the exponent. A quadratic curve was fitted to the fitted exponent vs. fitted bias data and resulted in values
\begin{equation}\label{equation_fitModelForExpVsBiasData}
B^* = 0.84A^2-1.1A+0.4
\end{equation}

 An important issue to address no is what is the validity of the fitted values?
Comparing the actual bias values to the prediction of the fitted quadratic curve \ref{equation_fitModelForExpVsBiasData} we get the difference curve as appears in Figure \ref{figure_diffFittedBiasVsModelBias}. The mean difference between model predictions and model fit is 0.0274. We see that for the beads 35-50 there is a disagreement between prediction of model \ref{equation_fitModelForExpVsBiasData} and model \ref{fitModel} (green dashed box in Figure \ref{figure_diffFittedBiasVsModelBias}). 
...
\label{figure_diffFittedBiasVsModelBias}
\end{figure}
...
% this section should be revised/deleted or moved to the begining of the simulation chapter 
\section{The effect of increasing the encounter distance}\label{section_effectOfIncreasingTheEncounterDistance}
