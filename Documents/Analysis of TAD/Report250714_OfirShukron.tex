...
We use the Rouse model to mathematically describe the polymer chain as a linear series of beads connected by harmonic springs. The stochastic differential equation describing the dynamics of bead $n$ in a chain of $N$ beads is 
\begin{equation*}
...
The flexibility of our framework enables us to simulate 3-dimensional single or ensemble of chains in open space or in 3 types of domains: a sphere, a cylinder, and between two infinite plates. As an extension to the classical Rouse model, we can also simulate the chain with variable minimal distance between adjacent beads.
To include the minimal distance, $l_0>0$, we modify the Rouse potential to read
\begin{equation*}
\phi_{Rouse}=-\frac{\kappa}{2}\sum_{n=1}^N\left((\|R_n-R_{n-1}\|-l_0)^2\right)
\end{equation*}
and simulate coupled (coordinate-wise) system of equations 
...
The encounter probability in a linear Rouse chain is given by 
\begin{equation}
